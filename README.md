...
tweak the CSS so that more than one frame is shown, and then set `centerCurrent` to
`true`. Whatever item is currently selected will be centered.
...
This functionality has been removed for the improved `centerCurrent` functionality above.
